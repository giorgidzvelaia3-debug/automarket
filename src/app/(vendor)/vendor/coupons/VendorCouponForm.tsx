...
export default function VendorCouponForm({
  categories,
  products,
}: {
  categories: Category[]
  ...
  const [scope, setScope] = useState<"VENDOR" | "CATEGORY" | "PRODUCT">("VENDOR")
          ...
          <div className="relative bg-white rounded-2xl shadow-xl max-w-md w-full p-6 space-y-4 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input type="text" value={code} onChange={(e) => setCode(e.target.value.toUpperCase())} placeholder="MYSTORE10" className={`${inputClass} font-mono`} />
                <button type="button" onClick={() => setCode(generateCode())} className="rounded-lg bg-gray-100 px-3 text-xs font-semibold text-gray-700 hover:bg-gray-200">Generate</button>
              </div>
            </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-0.5 bg-gray-100 rounded-lg p-0.5">
                  {(["PERCENTAGE", "FIXED"] as const).map((t) => (
                    <button key={t} type="button" onClick={() => setType(t)} className={`flex-1 px-3 py-1.5 rounded-md text-xs font-semibold transition-colors ${type === t ? "bg-white text-gray-900 shadow-sm" : "text-gray-500"}`}>
                      {t === "PERCENTAGE" ? "%" : "₾"}
                    </button>
                  ))}
                </div>
            ...
            <div>
              ...
              <select value={scope} onChange={(e) => setScope(e.target.value as never)} className={inputClass}>
                <option value="VENDOR">Entire store</option>
